...
\QQRow{一辺が $x\mathrm{cm}$ の正方形の面積を $y\mathrm{cm}^2$ とする。}{○}{半径が $x\mathrm{cm}$ の円の円周を $y\mathrm{cm}$ とする。}{×}
...
\TypeQuestionSingle{次の式で、指定された $x$ の値に対応する $y$ の値を求めなさい。}{atai}
\QQRow{$y=2x^2,\ x=3$}{$18$}{$y=3x^2,\ x=-2$}{$12$}
\QQRow{$y=-4x^2,\ x=2$}{$-16$}{$y=-x^2,\ x=-5$}{$-25$}
\QQRow{$y=\dfrac{1}{2}x^2,\ x=6$}{$18$}{$y=-\dfrac{1}{4}x^2,\ x=-8$}{$-16$}

\LessonPageBreak

\TypeQuestionDouble{$y$ が $x$ の二乗に比例し、次の条件を満たす。比例定数 $a$ を求めなさい。}{akettei}
...
\TypeQuestionDouble{$y$ が $x$ の二乗に比例し、次の条件を満たす。$y$ を $x$ の式で表しなさい。}{shikikettei}
...
\TypeQuestionDouble{次の数量関係について、$y$ を $x$ の式で表しなさい。}{bunsho}
