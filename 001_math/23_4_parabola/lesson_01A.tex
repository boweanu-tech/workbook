...
\TypeQuestionDouble{$y$ が $x$ の二乗に比例し、次の条件を満たす。指定された $x$ または $y$ の値に対応する値を求めなさい。}{gyakusan}
\QQ{$x=2$ のとき $y=8$ である。$x=5$ のときの $y$}{$50$}
\QQ{$x=-4$ のとき $y=12$ である。$x=8$ のときの $y$}{$48$}
\QQ{$x=-6$ のとき $y=-18$ である。$x=2$ のときの $y$}{$-2$}
\QQ{$x=2$ のとき $y=8$ である。$y=50$ のときの $x$}{$x=\pm5$}
\QQ{$x=3$ のとき $y=27$ である。$y=12$ のときの $x$}{$x=\pm2$}
\QQ{$x=-4$ のとき $y=-8$ である。$y=-18$ のときの $x$}{$x=\pm6$}
