...
\QQRow{$(1,0),\ (1,\sqrt3)$}{$60^\circ$}{$(1,1),\ (-1,1)$}{$90^\circ$}
\QQRow{$(3,4),\ (4,-3)$}{$90^\circ$}{$(2,-2),\ (-1,0)$}{$135^\circ$}
\QQRow{$(\sqrt3,1),\ (\sqrt3,-1)$}{$60^\circ$}{$(-2,2\sqrt3),\ (1,\sqrt3)$}{$60^\circ$}

\TypeQuestionDouble{次の2つのベクトルが垂直になるように、実数 $k$ の値を求めなさい。}{perpendicular_parameter}
\QQRow{$(2,3),\ (k,-2)$}{$k=3$}{$(k,4),\ (2,-3)$}{$k=6$}
\QQRow{$(k-1,2),\ (3,k)$}{$k=\dfrac35$}{$(1,k+2),\ (2k,-1)$}{$k=2$}
...
\QQ{$\mathrm{A}(-2,-1),\ \mathrm{B}(-2,5),\ \mathrm{C}(3,2)$}{$15$}
